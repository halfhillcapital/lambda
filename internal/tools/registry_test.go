...
// TestNewSchemas asserts every built-in tool ships a schema.
...
// TestReadOnlyToolsAutoAllow pins that the read-side tools never prompt.
func TestReadOnlyToolsAutoAllow(t *testing.T) {
	cases := []Tool{Read, Grep, Glob}
	for _, tool := range cases {
		t.Run(tool.Name(), func(t *testing.T) {
			if got := tool.Classify("{}"); got != AutoAllow {
				t.Errorf("%s.Classify = %v, want AutoAllow", tool.Name(), got)
			}
		})
	}
}
