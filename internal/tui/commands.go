...
type slashCommandResult struct {
	...
	notices    []string
	err        string
	// arg carries the raw argument text for commands that need one
	// (currently /discard <prefix>). Trimmed; "" means no arg.
	arg string
}
...
func (d slashCommandDispatcher) Dispatch(text string) slashCommandResult {
	...
	case "/discard":
		return slashCommandResult{kind: slashCommandDiscard, arg: args}
	case "/title":
		return slashCommandResult{kind: slashCommandTitle, arg: args}
	case "/model":
		return slashCommandResult{kind: slashCommandModel, arg: args}
	}
